...
XFormula = ~poly(day,2,raw=TRUE)*treatment + logseqdepth
...
pred_TJ1 <- constructGradient(m, 
                      focalVariable = "day", 
                      non.focalVariables = list(logseqdepth=list(1),treatment=list(3,"TJ1")), 
                      ngrid=gradientlength) %>%
             predict(m, Gradient = ., expected = TRUE)

pred_TJ2 <- constructGradient(m, 
                      focalVariable = "day", 
                      non.focalVariables = list(logseqdepth=list(1),treatment=list(3,"TJ2")), 
                      ngrid=gradientlength) %>%
             predict(m, Gradient = ., expected = TRUE)

Gradient_Time_AS=constructGradient(m, 
                      focalVariable = "day", 
                      non.focalVariables = list(logseqdepth=list(1),treatment=list(3,"TJ2")), 
                      ngrid=gradientlength)
predY_Sampling.time_AS=predict(m, Gradient = Gradient_Time_AS, expected = TRUE)
  
plotGradient(m, Gradient_Time_AS, pred=predY_Sampling.time_AS, measure="T",q = c(0.05, 0.5, 0.95), 
...
predY_TJ1 <- pred_TJ1 %>%
        ...
        mutate(treatment=rep("TJ1",gradientlength*1000)) %>%
...
predY_TJ2 <- pred_TJ2 %>%
        ...
        as.data.frame() %>%
        mutate(day=rep(gradient,1000)) %>%
        mutate(treatment=rep("TJ2",gradientlength*1000)) %>%
        pivot_longer(-c(day,treatment), names_to = "genome", values_to = "value")

predY <- rbind(predY_TJ1,predY_TJ2)
```
...
predY_summ <- predY %>%
  ...
  summarize(mean = mean(value, na.rm = TRUE),
            q0.05 = quantile(value, 0.05, na.rm = TRUE),
            q0.95 = quantile(value, 0.95, na.rm = TRUE))

# Overall trends
predY_asc <- predY_summ %>%
    pivot_wider(names_from = day, values_from = c(mean,q0.05,q0.95))  %>%
    filter(q0.95_0 < q0.05_28) %>%
    select(genome) %>%
    unique() %>% pull()

predY_desc <- predY_summ %>%
    pivot_wider(names_from = day, values_from = c(mean,q0.05,q0.95))  %>%
    filter(q0.05_0 > q0.95_28) %>%
    select(genome) %>%
    unique() %>% pull()
...
predY_dynamics <- predY_summ %>%
    ...
    mutate(TJ1 = ifelse(q0.9_7_TJ1 < q0.1_35_TJ1, "asc", "inv"))  %>%
    mutate(TJ1 = ifelse(q0.1_7_TJ1 > q0.9_35_TJ1, "dec", "inv"))  %>%
    mutate(TJ2 = ifelse(q0.9_7_TJ2 < q0.1_35_TJ2, "asc", "inv"))  %>%
    mutate(TJ2 = ifelse(q0.1_7_TJ2 > q0.9_35_TJ2, "dec", "inv"))  %>%
    mutate(dynamics = ifelse(any(!grepl("inv", c(TJ1, TJ2))), "variable", "invariable")) %>%
...
predY_contrasting <- predY_summ %>%
    ...
    mutate(TJ2 = ifelse(q0.1_7_TJ2 > q0.9_35_TJ2, "dec", "inv"))  %>%
    select(TJ1,TJ2) %>%
    mutate(comparison = ifelse(any(c(TJ1, TJ2) != TJ1), "contrasting", "aligned")) %>%
...
predT_TJ1 <- lapply(pred_TJ1, function(a) (a %*% m$Tr)/matrix(rep(rowSums(a), m$nt), ncol = m$nt)) %>%
        do.call(rbind,.) %>%
        as.data.frame() %>%
        mutate(day=rep(gradient,1000)) %>%
        mutate(treatment=rep("TJ1",gradientlength*1000)) %>%
...
predT_TJ2 <- lapply(pred_TJ2, function(a) (a %*% m$Tr)/matrix(rep(rowSums(a), m$nt), ncol = m$nt)) %>%
        do.call(rbind,.) %>%
        as.data.frame() %>%
        mutate(day=rep(gradient,1000)) %>%
        mutate(treatment=rep("TJ2",gradientlength*1000)) %>%
...
predT <- rbind(predT_TJ1,predT_TJ2)
```

```{r hmsc_temporal_predictions_traits_plot}
predT %>%
  filter(day == 0 | day == 7 | day == 14 | day == 21 | day == 28) %>%
  filter(value < 1) %>%
  filter(value > 0) %>%
  mutate(day = factor(day)) %>%
  ggplot(aes(x=value,y=fct_rev(day))) + 
      geom_boxplot(outlier.size=-1) +
      facet_grid(treatment ~ trait, scales = "free", space = "free") +
